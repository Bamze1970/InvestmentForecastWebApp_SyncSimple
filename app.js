...
function getStore(){ try { return { ...defaultStore(), ...JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}') }; } catch { return defaultStore(); } }
...
async function syncToCloud(store){
  ...
  const ref = doc(db, 'portfolios', activePortfolioId);
  try { await setDoc(ref, { store, updatedAt: serverTimestamp() }, { merge: true }); setStatus('Промените са качени в cloud portfolio: ' + activePortfolioId); } catch(e) { console.error(e); setStatus('Грешка при sync'); }
}

syncBtn.addEventListener('click', ()=>{ syncModal.classList.remove('hidden'); portfolioIdInput.value = getSettings().portfolioId || ''; });
closeSyncBtn.addEventListener('click', ()=> syncModal.classList.add('hidden'));
saveSyncBtn.addEventListener('click', ()=>{ const portfolioId = portfolioIdInput.value.trim(); saveSettings({ portfolioId }); syncModal.classList.add('hidden'); if(portfolioId) connectPortfolio(portfolioId); else setStatus('Portfolio ID е изчистен; работиш само локално.'); });
